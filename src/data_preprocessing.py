...
class AmazonFBAPreprocessor:
    ...
    def load_data(self):
        ...
        main_file = "./data/raw/inventory_data.csv"
    ...
    def create_output_tables(self, df, date_col, sku_col, qty_col, cost_col):
        ...
        logger.info(f"✅ Created daily_demand.csv: {len(daily_demand):,} records")
        ...
        sku_master.columns = ['sku_id', 'unit_cost', 'category', 'abc_class', 'fc', 'storage']
        sku_master.to_csv(self.processed_dir / 'sku_master.csv', index=False)
        logger.info(f"✅ Created sku_master.csv: {len(sku_master):,} SKUs")
        
        # Purchase orders
        po = self.generate_purchase_orders(df, sku_col)
        po.to_csv(self.processed_dir / 'purchase_orders.csv', index=False)
        logger.info(f"✅ Created purchase_orders.csv: {len(po):,} orders")
...
def main():
    ...
    tables = processor.create_output_tables(df, date_col, sku_col, qty_col, cost_col)
    
    logger.info("✅ PREPROCESSING COMPLETE")
    logger.info(f"Created files in: {processor.processed_dir}")
